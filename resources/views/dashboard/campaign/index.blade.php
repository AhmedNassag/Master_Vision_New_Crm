    ...
    <div class="d-flex flex-column flex-column-fluid">
                ...
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                        ...
                        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                            ...
                            <li class="breadcrumb-item text-muted">{{ trans('main.Campaigns') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div id="kt_app_content_container" class="app-container container-xxl">
                                    ...
                                    <form action="{{ route('campaign.index') }}" method="get">
                                        <div class="px-7 py-5">
                                            ...
                                            <div class="mb-10">
                                                ...
                                                <input type="text" class="form-control form-control-solid" placeholder="{{ trans('main.url') }}" name="url" value="{{ @$url }}" />
                                            </div>
                                            <div class="mb-10">
                                                <label class="d-flex align-items-center fs-5 fw-semibold mb-2">
                                                    <span class="required">{{ trans('main.ContactSource') }}</span>
                                                </label>
                                                <select name="contact_source_id" data-control="select2" data-dropdown-parent="#kt_app_content" class="form-select form-select-solid">
                                            ...
                                            <div class="mb-10">
                                                ...
                                                <select name="interest_id" data-control="select2" data-dropdown-parent="#kt_app_content" class="form-select form-select-solid">
                                                    <option value="">{{ trans('main.Select') }}...</option>
                                                    <?php $subActivities = \App\Models\SubActivity::get(['id','name']); ?>
                                                    @foreach($subActivities as $subActivity)
                                                        <option value="{{ @$subActivity->id }}" {{ @$subActivity->id == @$interest_id ? 'selected' : '' }}>{{ @$subActivity->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    @if($data->count() > 0)
                                        @foreach ($data as $key=>$item)
                                            <tr>
                                                ...
                                                <td class="text-center">@if($item->url) {{ @$item->url . '?campaign_id=' . @$item->id }} @endif</td>
                                                <td class="text-center">
                                            ...
                                            @include('dashboard.campaign.editModal')
                                            @include('dashboard.campaign.deleteModal')
                                        @endforeach
                                    @else
                                        <tr>
                ...
                @include('dashboard.campaign.addModal')
