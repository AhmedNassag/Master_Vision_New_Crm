...
<div class="modal fade" id="delete_modal_{{ @$item->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form class="form" action="{{ route('activity.destroy', 'test') }}" method="POST" enctype="multipart/form-data" id="delete-form-{{ @$item->id }}">
                {{ method_field('DELETE') }}
                @csrf
                <div class="modal-header">
                    <h2>{{ trans('main.Delete') }} {{ trans('main.Activity') }}</h2>
                ...
                <div class="modal-footer flex-center">
                    ...
                    <button type="submit" class="swal2-confirm btn fw-bold btn-danger" id="submitDeleteButton-{{ @$item->id }}">
                        <span class="indicator-label">{{ trans('main.Delete') }}</span>
                    </button>
                </div>
            </form>
        </div>
