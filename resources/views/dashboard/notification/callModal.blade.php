            ...
            <form class="form" action="{{ route('meeting.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body py-10 px-lg-17">
                    ...
                    <div id="interests_ids" class="col-md-12 fv-row">
                        ...
                        <select name="interests_ids[]" multiple data-control="select2" data-dropdown-parent="#interests_ids" class="form-select form-select-solid" required>
                            ...
                            @foreach($subActivities as $subActivity)
                                <option value="{{ @$subActivity->id }}" {{ in_array(@$subActivity->id, $item->meeting->interests_ids) ? 'selected' : '' }}>{{ @$subActivity->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!-- type -->
                    <div id="type" class="col-md-12 fv-row">
                        ...
                        <select name="type" data-control="select2" data-dropdown-parent="#type" class="form-select form-select-solid" required>
                            <option value="call" {{ $item->meeting->type == 'call' ? 'selected' : '' }}>{{ trans('main.Call') }}</option>
                            <option value="meeting" {{ $item->meeting->type == 'meeting' ? 'selected' : '' }}>{{ trans('main.Meeting') }}</option>
                        </select>
                    </div>
                    <!-- meeting_place -->
                    <div id="meeting_place" class="col-md-12 fv-row">
                        ...
                        <select name="meeting_place" data-control="select2" data-dropdown-parent="#meeting_place" class="form-select form-select-solid" required>
                            <option value="in" {{ $item->meeting->meeting_place == 'in' ? 'selected' : '' }}>{{ trans('main.Incoming') }} / {{ trans('main.Inner') }}</option>
                            <option value="out" {{ $item->meeting->meeting_place == 'out' ? 'selected' : '' }}>{{ trans('main.Outcoming') }} / {{ trans('main.Outing') }}</option>
                        </select>
                    </div>
                    <!-- meeting_date -->
                    <div class="col-md-12 fv-row">
                        <label class="fs-5 fw-semibold mb-2 required">{{ trans('main.Meeting Date') }}</label>
                        <input type="date" class="form-control form-control-solid" placeholder="{{ trans('main.Meeting Date') }}" value="{{ date('Y-m-d') }}" name="meeting_date" required/>
                    </div>
                    <!-- revenue -->
                    <div class="col-md-12 fv-row">
                        <label class="fs-5 fw-semibold mb-2 required">{{ trans('main.Revenue') }}</label>
                        <input type="number" class="form-control form-control-solid" placeholder="{{ trans('main.Revenue') }}" value="{{ @$item->meeting->revenue }}" name="revenue" required/>
                    </div>
                    <!-- reply_id -->
                    ...
                    <!-- follow_date -->
                    <div class="col-md-12 fv-row">
                        <label class="fs-5 fw-semibold mb-2 required">{{ trans('main.Follow Date') }}</label>
                        <input type="date" class="form-control form-control-solid" placeholder="{{ trans('main.Follow Date') }}" value="{{ old('follow_date') }}" name="follow_date" required/>
                    </div>
                    <!-- notes -->
                    <div class="col-md-12 fv-row">
                    ...
                    <div class="row mb-5">
                        <div class="col-md-12 fv-row">
                            <input class="form-control form-control-solid" type="hidden" name="contact_id" value="{{ @$item->meeting->contact_id }}">
                        </div>
                    </div>
                </div>
