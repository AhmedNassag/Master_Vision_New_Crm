                        ...
                        <div class="card card-flush">
                            ...
                            <div class="card-body pt-5 px-3">
                                ...
                                <div class="d-flex flex-column gap-5">
                                    ...
                                    <!--begin::Contact group-->
                                    <div class="d-flex flex-stack">
                                        <a href="{{ route('contact.index', ['status' => 'archived']) }}" class="fs-6 fw-bold text-gray-800 text-hover-primary">{{ trans('main.archived') }}</a>
                                        <?php
                                            if(Auth::user()->roles_name[0] == "Admin")
                                            {
                                                $new_contacts = App\Models\Contact::where('is_trashed','!=' ,1)->where('is_active', '!=' ,0)->where('status', 'archived')->count();
                                            }
                                            else if(Auth::user()->roles_name[0] != "Admin" && Auth::user()->employee->has_branch_access == 1)
                                            {
                                                $new_contacts = App\Models\Contact::where('is_trashed','!=' ,1)->where('is_active', '!=' ,0)
                                        ...
                                        ?>
                                        <div class="badge badge-light-danger">{{ @$new_contacts}}</div>
                                        <form action="{{ route('contact.index') }}" method="get" id="new_contacts" style="display: none">
                                            <input type="hidden" name="status" value="new" />
                                        </form>
                                    </div>
                                ...
                                <div id="multi_selected_div" class="d-flex justify-content-sm-start  justify-content-lg-end flex-wrap" data-kt-customer-table-toolbar="base" style="display: none">
                                    <div class="m-1">
                                        @can('إرسال رسائل جهات الإتصال')
                                            ...
                                            <button id="btn_sendTemplateMessage_selected" type="button" class="btn btn-sm btn-light-success" data-bs-toggle="modal" data-bs-target="#sendTemplateMessage_selected" style="display: none">
                                                <i class="ki-duotone ki-whatsapp ">
                                                    <span class="path1"></span>
                                                    <span class="path2"></span>
                                                </i>
                                                {{ trans('main.Send') }} {{ trans('main.TemplateMessage') }}
                                            </button>
                                        @endcan
                                    </div>
                                ...
                                <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="example1">
                                    ...
                                    <tbody class="fw-semibold text-gray-600">
                                        ...
                                        <tr>
                                                ...
                                                <a href="{{ route('contact.show', $item->id) }}">
                                                    ...
                                                    @elseif(@$item->status == 'archived')
                                                        <label class="badge badge-light-danger">
                                                            {{ app()->getLocale() == 'ar' ? 'مؤرشف' : 'Archived' }}
                                                        </label>
                                                    @else
                                            ...
                                            @include('dashboard.contact.sendTemplateMessageSelectedModal')
                                        @endforeach
                                        @else
                                        <tr>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('select[name="interest_id"]').on('change', function() {
            var interest_id = $(this).val();
            if (interest_id) {
                $.ajax({
                    url: "{{URL::to('admin/serviceByInterestId')}}/" + interest_id,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
