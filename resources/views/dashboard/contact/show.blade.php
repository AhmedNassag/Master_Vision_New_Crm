                                    ...
                                    <div id="kt_customer_view_details" class="collapse show">
                                        <div class="py-5 fs-6">
                                            ...
                                            @if($item->campaign)
                                                <div class="row mb-7">
                                                    <div class="col-5">
                                                        <div class="fw-bold">{{ trans('main.Campaign') }}:</div>
                                                    </div>
                                                    <div class="col-7">
                                                        <div class="text-gray-600">{{ @$item->campaign->name }}</div>
                                                    </div>
                                                </div>
                                            @endif
                                            <!--begin::Details item-->
...
@section('js')
            ...
            $('select[name="invoice[interest_id]"]').on('change', function(){
                ...
                if (activity_id) {
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function(data){
                            // Clear the service select field and add the default "Select..." option
                            $('select[name="invoice[service_id]"]').empty().append('<option value="">{{ trans('main.Select') }}...</option>');
                            
                            // Append the retrieved service options
                            $.each(data, function(key, value) {
                                $('select[name="invoice[service_id]"]').append('<option class="form-control" value="'+ value["id"] +'">' + value["name"] + '</option>');
                            });
                        }
                    ...
                    // If no activity is selected, empty the service dropdown and add the default "Select..." option
                    $('select[name="invoice[service_id]"]').empty().append('<option value="">{{ trans('main.Select') }}...</option>');
                    console.log('not work');
                }
            });
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $.ajax({
                url: "https://nbenewcopy.nationalegypt.com/api/getPackagesForApi",
                type:"GET",
                dataType:"json",
                success:function(data){
                    $('select[name="package_id"]').empty();
                    $.each(data,function(key,value) {
                        $('select[name="package_id"]').append('<option class="form-control" value="'+ value["id"] +'">' + value["title"] + '</option>');
                    });
                }
            });
        });
