...
<div class="modal fade" id="delete_modal_{{ @$item->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form class="form" action="{{ route('major.destroy', 'test') }}" method="POST" enctype="multipart/form-data" id="delete-form-{{ @$item->id }}">
                {{ method_field('Delete') }}
                @csrf
                <div class="modal-header">
                    <h2>{{ trans('main.Delete') }} {{ trans('main.Major') }}</h2>
