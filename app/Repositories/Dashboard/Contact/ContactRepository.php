...
class ContactRepository implements ContactInterface
{
    ...
    //     // Base query
    //     $query = Contact::where('is_trashed', '!=', 1)
    //         ->with(['media', 'contactSource', 'city', 'area', 'contactCategory', 'activity', 'subActivity', 'employee']);
    ...
    //         // You can apply additional admin-specific filters here if needed
    //     } else if (Auth::user()->roles_name[0] != "Admin" && Auth::user()->employee->has_branch_access == 1) {
    //         $query->where(function ($q) {
    //             $q->whereRelation('createdBy', 'branch_id', auth()->user()->employee->branch_id)
    ...
    //     // Execute and paginate the query
    //     $datas = $query->orderBy('id', 'desc')->paginate($perPage)->appends(request()->query());
    ...
    //         'activity_id' => function ($q) use ($request) {
    //             if ($request->filled('activity_id') && is_array($request->activity_id)) {
    //                 return $q->whereIn('activity_id', $request->activity_id);
    //             }
    //         },
    //         'interest_id' => null,
    //         'branch_id' => null,
    ...
    //         'area_id' => null,
    //         'industry_id' => null,
    //         'reply_id' => function ($q) use ($request) {
    //             $q->whereHas('meetings', function ($query) use ($request) {
    //                 $query->where('reply_id', $request->reply_id);
    //             });
    //         },
    //         'major_id' => null,
    //         'campaign_id' => null,
    //         'custom_attributes' => function ($q) use ($request) {
    ...
    //                 $condition($query);
    //             } else if ($condition) {
    //                 $query->where($field, $condition, '%' . $request->$field . '%');
    //             } else {
    ...
    public function index($request)
    {
        ...
        if(Auth::user()->roles_name[0] == "Admin")
        {
            ...
            ->when(!$request->has('status'), function ($q) {
                return $q->whereNotIn('status', ['converted', 'archived']);
            })
    ...
    {
        $response = Http::get('https://nbenewcopy.nationalegypt.com/api/getPackagesForApi');
        ...
        {
            return back()->with('error', 'Unable to fetch packages from API.');
        }
    ...
    public function store($request)
    {
        try {
            ...
            $inputs['mobile']            = preg_replace('/^\+2|\s+/', '', $request->input('mobile'));
