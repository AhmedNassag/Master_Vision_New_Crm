...
class ReportController extends Controller
{
    ...
    {
        $this->middleware('permission:عرض تقارير الإيصالات', ['only' => ['payments', 'paymentsReport']]);
    ...
    {
        /*
        $invoices = Invoice::with('recorderReminders')
            ->withSum('recorderReminders', 'expected_amount') // Summing the payment amounts
            ->get();

        return $invoices;
        */
        return View('dashboard.report.payments');
    }
    ...
    public function paymentsReport(Request $request)
    {
        try {
            ...
            {
                $data = ReorderReminder::with('customer','activity','interest','invoice','updatedBy','createdBy')

                ->when($request->invoice_number != null,function ($q) use($request){
                    return $q->whereRelation('invoice','invoice_number', $request->invoice_number);
                })
                ->when($request->activity_id != null, function ($q) use ($request) {
                    return $q->whereRelation('invoice','activity_id', $request->activity_id);
                })
                ->when($request->interest_id != null, function ($q) use ($request) {
                    return $q->whereRelation('invoice','interest_id', $request->interest_id);
                })
                ->when($request->service_id != null, function ($q) use ($request) {
                    return $q->whereRelation('invoice','service_id', $request->service_id);
                })
                ->when($request->invoice_created_by != null,function ($q) use($request){
                    return $q->whereRelation('invoice','created_by', $request->invoice_created_by);
                })
                ->when($request->invoice_from_date != null,function ($q) use($request){
                    return $q->whereRelation('invoice','invoice_date', '>=', $request->invoice_from_date);
                })
                ->when($request->invoice_to_date != null,function ($q) use($request){
                    return $q->whereRelation('invoice','invoice_date', '>=', $request->invoice_to_date);
                })

                ->when($request->reorder_reminder_number != null,function ($q) use($request){
                    return $q->where('reorder_reminder_number', $request->reorder_reminder_number);
                })
                ->when($request->created_by != null,function ($q) use($request){
                    return $q->where('created_by', $request->created_by);
                })
                ->when($request->updated_by != null,function ($q) use($request){
                    return $q->where('updated_by', $request->updated_by);
                })
                ...
                ->when($request->from_date != null,function ($q) use($request){
                    return $q->whereDate('reminder_date', '>=', $request->from_date);
                })
                ->when($request->to_date != null,function ($q) use($request){
                    return $q->whereDate('reminder_date', '<=', $request->to_date);
                })
            ...
            //     ->withSum('recorderReminders', 'expected_amount')
            //     ->get();
            //     */
            ...
            foreach ($data as $group) {
                $total_amount += $group->first()->invoice->total_amount ?? 0;
                $amount_paid  += $group->first()->invoice->amount_paid ?? 0;
                $debt         += $group->first()->invoice->debt ?? 0;
            }

            return view('dashboard.report.payments',compact('data'))
            ->with([
                'invoice_number'          => $request->invoice_number,
                'activity_id'             => $request->activity_id,
                'interest_id'             => $request->interest_id,
                'service_id'              => $request->service_id,
                'invoice_created_by'      => $request->invoice_created_by,
                'invoice_from_date'       => $request->invoice_from_date,
                'invoice_to_date'         => $request->invoice_to_date,
                'reorder_reminder_number' => $request->reorder_reminder_number,
