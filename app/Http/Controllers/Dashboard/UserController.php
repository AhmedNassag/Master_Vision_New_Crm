...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $validator = Validator::make($request->all(),[
                'name'       => 'required',
                'email'      => 'required|email|unique:users,email,NULL,id,deleted_at,NULL|unique:employees,email,NULL,id,deleted_at,NULL',
                'mobile'     => 'required|unique:users,mobile,NULL,id,deleted_at,NULL|unique:employees,mobile,NULL,id,deleted_at,NULL',/*|regex:/^0\d{9,}$/*/
    ...
    public function update(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(),[
                ...
                'mobile'     => 'required|unique:users,mobile,'.$request->id,'|unique:employees,mobile,'.$request->id,/*|regex:/^0\d{9,}$/*/
                'branch_id'  => 'required|integer|exists:branches,id',
                'dept'       => 'required|integer|exists:departments,id',
                'status'     => 'nullable',
                'roles_name' => 'required',
    ...
    {
        try {
            $employees = DB::table('employees')->where('deleted_at',null)->where('hidden',0)->where('branch_id', $id)->select('name', 'id')->get();
            if($employees)
            {
    ...
    {
        try {
            $employees = DB::table('employees')->where('deleted_at',null)->where('hidden',0)->where('dept', $id)->select('name', 'id')->get();
            if($employees)
            {
                return response()->json($employees);
            }
