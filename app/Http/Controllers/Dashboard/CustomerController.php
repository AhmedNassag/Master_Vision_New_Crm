...
class CustomerController extends Controller
{
    ...
    public function cancelInvoice(Request $request)
    {
        // dd($request->id);
            try {
                ...
                DB::table('cancelled_invoices')->insert($cancelledInvoiceData);


                $recorderReminders = $invoice->recorderReminders;
                if($recorderReminders && $recorderReminders->count() > 0)
                {
                    foreach($recorderReminders as $recorderReminder)
                    {
                        $cancelled_reorder_reminders = [
                            'reminder_type'                => $recorderReminder->reminder_type,
                            'reminder_date'                => $recorderReminder->reminder_date,
                            'is_completed'                 => $recorderReminder->is_completed,
                            'expected_amount'              => $recorderReminder->expected_amount,
                            'invoice_id'                   => $recorderReminder->invoice_id,
                            'customer_id'                  => $recorderReminder->customer_id,
                            'interest_id'                  => $recorderReminder->interest_id,
                            'activity_id'                  => $recorderReminder->activity_id,
                            'reorder_reminder_number'      => $recorderReminder->reorder_reminder_number,
                            'receiptnumber'                => $recorderReminder->receiptnumber,
                            'notes'                        => $recorderReminder->notes,
                            'created_by'                   => $recorderReminder->created_by,
                            'updated_by'                   => $recorderReminder->updated_by,
                            'created_at'                   => $recorderReminder->updated_by,
                            'updated_at'                   => $recorderReminder->updated_at,
                        ];
                        DB::table('cancelled_reorder_reminders')->insert($cancelled_reorder_reminders);
                        $recorderReminder->delete();
                    }
                }
                ...
                if($customer->invoices()->count() == 1)
                {
                    ...
                    $contact->update(['status'=>'archived','customer_id'=>null]);
                    $customer->delete();
    ...
    {
        try {
            $invoice = Invoice::find($request->reminder['invoice_id']);
            if (!$invoice)
            {
                return redirect()->back()->withErrors(['error' => 'Invoice not found'])->withInput();
            }
            $totalAmount     = $invoice->total_amount;
            $totalReorders   = ReorderReminder::where('invoice_id', $request->reminder['invoice_id'])->sum('expected_amount');
            $remainingAmount = $totalAmount - $totalReorders;

            $validator = Validator::make($request->all(), [
                'reminder'                 => ['required','array','min:1'],
                'reminder.reminder_date'   => 'required|date',
                'reminder.expected_amount' => ['required', 'numeric', function($attribute, $value, $fail) use ($remainingAmount) {
                    if ($value > $remainingAmount) {
                        $fail(trans('main.The expected amount must be less than or equal to the total debt of ') . '(' . number_format($remainingAmount,0) . ')');
                    }
                }],
                'reminder.customer_id'     => 'required',
                'reminder.invoice_id'      => 'required',
                'reminder.notes'           => 'nullable',
            ]);
            if ($validator->fails())
            {
                session()->flash('error');
            ...
            $data['interest_id']             = $invoice->interest_id;
            $data['activity_id']             = $invoice->activity_id;
            $data['created_by']              = auth()->user()->employee->id;
            // $data['reorder_reminder_number'] = '---';
            ...
            session()->flash('success');
            return redirect()->back()->with('success', 'لقد تمت العملية بنجاح');
    ...
    {
        try {
            $recorderReminder = ReorderReminder::findOrFail($request->id);
            if (!$recorderReminder)
            {
                return redirect()->back()->withErrors(['error' => 'ReorderReminder not found'])->withInput();
            }
            if ($recorderReminder->is_completed == 1)
            {
                return redirect()->back()->withErrors(['error' => 'لا يمكن التعديل لأن التحصيل تم دفعه بالفعل'])->withInput();
            }

            $invoice = Invoice::findOrFail($recorderReminder->invoice_id);
            if (!$invoice)
            {
                return redirect()->back()->withErrors(['error' => 'Invoice not found'])->withInput();
            }
            $totalAmount     = $invoice->total_amount;
            $totalReorders   = ReorderReminder::where('invoice_id', $invoice->id)->sum('expected_amount');
            $remainingAmount = $totalAmount - $totalReorders;

            $validator = Validator::make($request->all(), [
                'reminder_date'   => 'required|date',
                'expected_amount' => ['required', 'numeric', function($attribute, $value, $fail) use ($remainingAmount) {
                    if ($value > $remainingAmount) {
                        $fail(trans('main.The expected amount must be less than or equal to the total debt of ') . '(' . number_format($remainingAmount,0) . ')');
                    }
                }],
