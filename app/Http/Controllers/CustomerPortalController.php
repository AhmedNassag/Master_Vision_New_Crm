...
class CustomerPortalController extends Controller
{
    ...
    public function postReply(Ticket $ticket,Request $request)
    {
        try {
            ...
            if ($notifiable) {
                Mail::to($notifiable->email)->send(new TicketReplyMail($ticketRecord, $request->notes));
    ...
    public function showCreateTicket()
    {
        try {
            $user = Auth::user();
            if($user->activity_id != null)
            {
                $activity = Activity::find($user->activity_id);
                if ($user->invoices()->exists()) {
                    $sub_activities = SubActivity::whereIn('id', function($query) use ($user) {
    ...
    public function storeTicket(Request $request)
    {
        try {
            ...
            Mail::to(User::/*hidden()->*/first()->email)->send(new TicketSendMail($ticket, $request->notes));
            session()->flash('success');
