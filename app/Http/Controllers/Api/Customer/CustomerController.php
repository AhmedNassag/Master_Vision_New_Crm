...
class CustomerController extends Controller
{
    ...
    {
        $this->middleware(['auth:customer_api']);
    }
    ...
    public function home(Request $request)
    {
        try {
            ...
            $customer_details['Parent Customer']   = $customer->parent ? $customer->parent->name : '';
    ...
    public function tickets(Request $request)
    {
        try {
            ...
            where(function ($query) use ($auth_id) {
                $query->where('customer_id', $auth_id) // Direct customer
                    ->orWhereHas('customer', function ($query) use ($auth_id) {
                        $query->where('parent_id', $auth_id); // Customer's parent
                    })
                    ->orWhereHas('customer.customers', function ($query) use ($auth_id) {
                        $query->where('id', $auth_id); // Customer in the customer's 'customers'
                    });
            })
            /**/
            /*
            ...
            */
            ->with(['customer','activity','subActivity','media','agent','logs','logs.media'])->orderBy('created_at', 'desc')->paginate(20);
            if ($tickets) {
...
//                $sub_activities = SubActivity::where('activity_id', $activity->id)->get(['id', 'name']);
            $sub_activities = SubActivity::where('id', $auth_customer->interest_id)->get(['id', 'name']);
            }
            else
            {
    ...
    public function storeTicket(Request $request)
    {
        try {
            ...
            [
                ...
                'photo'       => 'nullable|file|max:10000'/*|mimes:png,jpg,jpeg,webp*/,
            ]);
            if ($validator->fails())
            {
                return response()->json($validator->errors(), 422);
    ...
    public function showTicket($id)
    {
        try {
            ...
            $ticket = Ticket::with(['customer','activity','subActivity','media','agent','logs','logs.media'])->findOrFail($id);
            if (!$ticket) {
                return $this->apiResponse(null, 'There Is No Data found', 404);
            }
            return $this->apiResponse($ticket, 'The Data Returns Successfully', 200);
    ...
    public function replyTicket(Ticket $ticket,Request $request)
    {
        try {
            ...
            $service       = new TicketService();
            $service->replyToTicket($ticket,/*auth()->user()*/$auth_customer,'customer',$request->reply,$request->photo);
