...
class HomeController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            if (count($emps)>0) {
                $contacts_count=Contact::whereIn("created_by",$emps)->count();
            ...
            $monthReminders = ReorderReminder::whereYear('reminder_date', Carbon::now()->year)
                ...
                ->count();
            */
            $todayReminders = ReorderReminder::
            ...
            $todayReminders_amount = ReorderReminder::
            whereDate('reminder_date', Carbon::today())
            /*->orWhere(function ($query) {
                $query->whereDate('reminder_date', '<', Carbon::today())
                    ->where('is_completed', 0);
            ...
            $delayReminders = ReorderReminder::
            whereDate('reminder_date', '<' ,Carbon::today())->where('is_completed', 0)
            /*->orWhere(function ($query) {
                $query->whereDate('reminder_date', '<', Carbon::today())
                    ->where('is_completed', 0);
            })*/
            ...
            $delayReminders_amount = ReorderReminder::
            whereDate('reminder_date', '<' ,Carbon::today())->where('is_completed', 0)
            /*->orWhere(function ($query) {
                $query->whereDate('reminder_date', '<', Carbon::today())
                    ->where('is_completed', 0);
            ...
            $monthReminders_amount = ReorderReminder::whereYear('reminder_date', Carbon::now()->year)
            ->whereMonth('reminder_date', Carbon::now()->month)
            ->orWhere(function ($query) {
                $query->whereMonth('reminder_date', '<', Carbon::now()->month)
                    ->where('is_completed', 0);
            ...
            return view('home', compact(
                ...
                'did_meetings',
                'todayReminders',
                'todayReminders_amount',
                'monthReminders',
                'monthReminders_amount',
                'delayReminders',
                'delayReminders_amount',
                'todayFollowUps',
                'monthFollowUps',
                'todayBirthdays',
                'customers',
