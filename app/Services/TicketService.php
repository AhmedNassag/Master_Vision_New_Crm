...
class TicketService
{
    ...
    public function changeStatus(Ticket $ticket, $newStatus)
    {
        return DB::transaction(function () use ($ticket, $newStatus)
        {
            ...
            // Send notification about the status change
            $notifiable = Customer::find($ticket->customer_id);
            if ($notifiable) {
                // Dynamic title based on the status change
                $title = 'Ticket Status Changed: ' . trans(  $newStatus);
                $notifiable->notify(new TicketReplyNotification($ticket, $title));
            }

            // Optionally, you can add additional logic here, e.g., updating timestamps, etc.
